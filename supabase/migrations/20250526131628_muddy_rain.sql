/*
  ...
  1. New Columns
    - insurance_documents (JSONB): Track insurance documents (RC Pro, Decennale)
    - claims (JSONB): Track claims history and current claims
    - insee_verification (JSONB): Store INSEE verification data
    - administrative_documents (JSONB): Track administrative documents (KBIS, URSSAF, etc.)
    - quality_tracking (JSONB): Track company quality metrics
    - alerts (JSONB): Store system alerts
    - reminders (JSONB): Store reminders for document renewals

  2. Indexes
    - Added GIN indexes with jsonb_path_ops for efficient JSONB querying
    
  3. Functions and Triggers
    - Added document expiration check function and trigger
    
  4. Security
    - Added policies for commercial and mandatary roles
*/
